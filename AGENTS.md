...
- OTP（交通查询）不是日常开发的硬依赖：`dev.sh` 默认 `MEGURI_TRANSIT_MODE=live` 指向本地 OTP（:8081），未启动时 Navigator 自动降级为估算段。graph 构建：`otp/download.sh && otp/build.sh`（幂等，吃内存，docker VM ≥ 6GB）。
