...
**各维度分别考什么**（每个维度的输入 / 输出 / 打分算法）：

- **Scout 检索命中率**
  - 输入：作品名 + 地区 + 假仓库材料（若干真实圣地，含区域外的干扰项）
  - 输出：工具返回的候选圣地 id 列表
  - 算法：命中率 = |期望 id ∩ 实际 id| ÷ |期望 id|；特例——期望为空时，实际也必须为空（地区填"东京"的反例题：返回任何点都算错）。地理归属是标准答案的独立依据，不抄系统现状
- **Planner 切天质量**
  - 输入：候选圣地列表 + 天数
  - 输出：按天组织的行程快照
  - 算法：四条布尔规则全真才过——天数等于请求天数；每天至少一站；候选点一个不许丢；最近邻排序的总距离 ≤ 固定种子随机基线（把站点顺序用固定种子打乱 20 次取平均路程，Planner 的排法必须更好，证明排序真在优化而非随机）
- **Navigator 时间可行性**
  - 输入：候选圣地列表 + 天数 + 假交通数据（固定耗时）
  - 输出：每站的计划到达时刻
  - 算法：每站都有时刻记录；时刻是 "HH:MM" 格式；天内时刻单调不倒流
- **Storyteller 讲解接地**
  - 输入：候选圣地列表（带来源 origin 字段）
  - 输出：每站的讲解文本 + citation
  - 算法：每条讲解 0/1——文本必须出现本站站名，且 citation 的来源署名必须等于该站的 origin；取各条均分
- **e2e 端到端清单**
  - 输入：一条自然语言消息（"宇治三天京吹"）+ 全套假环境
  - 输出：一次完整请求的响应 + 落库数据 + trace 文件
  - 算法：六项谓词各 0/1，全过才算过——有回复、天数对、有交通段、有时刻、讲解带署名、trace 四类埋点齐全
- **work_resolve 作品名解析**
  - 输入：一个作品名查询词（唯一打真实数据库的维度：19144 条真实 Bangumi 索引灌进测试库）
  - 输出：解析出的作品列表（subjectID）
  - 算法：8 个 case 全对才算过——官方全名（"轻音少女"命中三部曲且第一季居首）、具体季、带空白、日文原名、错字救回（"吹响吧上低音号"）、俗名如实不命中（"京吹"）、未收录返回空、歧义取最短名（"你的名字"）。这组题同时用来标定 pg_trgm 的相似度阈值：阈值不是拍的，是调到这组题全对为止。
