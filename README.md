...
**数据层架构（用户拍板，最终版）**：
- **作品 ID 空间来自 Bangumi 全量索引**：`python -m app.ingest_bangumi` 用 Bangumi v0 API（自定义 UA、限速 ≤2 req/s、按年 checkpoint 断点续传）拉 1990 年后全部动画 → `data/works/anime-1990plus.json`（Git 里的源 artifact）；再经 `python -m app.rag.ingest_works` 幂等 upsert 进 **anime_works 表**（DB 是运行时的服务层，JSON 可重建）。运行流程：用户 prompt →（LLM）解析出作品名 → **anime_works 表解析 subjectID**（norm 列 GIN trigram 索引：子串精确匹配优先，trigram similarity 模糊兜底错字；多作品命中全部返回，如"轻音少女"→第一季+第二季+剧场版）→ **实时**调 anitabi `/bangumi/{id}/lite` 拿圣地数据。
...
- **golden 数据集** `eval/datasets/*.jsonl`，按 Agent 组织（scout/planner/navigator/storyteller/e2e）+ 解析层（work_resolve 作品名解析准确率，打真实 PG 的 anime_works 表，标定 pg_trgm 阈值用），内容用真实已抓取数据构造（anitabi 京吹宇治地标、bgm.tv 条目）。扩数据集 = 追加 JSONL 行（字段见 `eval/harness.py` 的读取处）。
- **指标**：Scout 检索命中率（期望 id 子集的命中比例，数据集每例带 `rationale` 说明期望的独立依据）；Planner 规则（天数正确/每天非空/全覆盖/最近邻不劣于固定种子随机基线）；Navigator 时间可行性（checks 完整/到达时刻单调）；Storyteller **讲解接地 narration_grounded**（讲解文本含站名、来源署名与站点 origin 一致；生成式是否编造事实是真 LLM judge 的事，已 deferred）；e2e 检查清单（回复/行程/legs/checks/讲解/trace 覆盖，RuleJudge 布尔评分）。
